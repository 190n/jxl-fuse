...
fn jxlToJpeg(jxl_buffer: []const u8, allocator: std.mem.Allocator) ![]u8 {
    const Buffers = struct {
        ...
        pub fn provideMoreRoom(self: *Buffers, dec: *libjxl.Decoder) ![]u8 {
            self.release(dec);
            self.full_buffer = try self.allocator.realloc(self.full_buffer, @max(4096, 2 * self.full_buffer.len));
    ...
    try decoder.setJpegBuffer(try bufs.provideMoreRoom(&decoder));
    ...
    while (true) {
        ...
        switch (status) {
            ...
            .jpeg_need_more_output => {
                const buf = try bufs.provideMoreRoom(&decoder);
