 ...
 */
bool DecodeJpegXlOneShot(const uint8_t *jxl, size_t size, std::vector<float> *pixels, size_t *xsize,
	...
	for (;;) {
		...
		if (status == JXL_DEC_ERROR) {
			...
			printf("number of bytes just added: %zu\n",
			...
			printf("%zu bytes were not written. %zu-byte buffer now holds %zu bytes of jpeg data\n",
			    unwritten_bytes, jpeg_buffer.size(), amount_in_jpeg_buffer);
			if (jpeg_buffer.size() >= 8) {
				printf("first few bytes of buffer: %02x%02x%02x%02x%02x%02x%02x%02x\n",
				    jpeg_buffer[0], jpeg_buffer[1], jpeg_buffer[2], jpeg_buffer[3], jpeg_buffer[4],
				    jpeg_buffer[5], jpeg_buffer[6], jpeg_buffer[7]);
			}
			jpeg_buffer.resize(jpeg_buffer.size() * 2);
			last_chunk_size = jpeg_buffer.size() - amount_in_jpeg_buffer;
			if (JXL_DEC_SUCCESS
			    != JxlDecoderSetJPEGBuffer(
			        dec.get(), jpeg_buffer.data() + amount_in_jpeg_buffer, last_chunk_size)) {
				fprintf(stderr, "JxlDecoderSetJPEGBuffer failed\n");
				return false;
			}
